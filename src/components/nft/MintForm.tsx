...
/**
 * Uploads a file directly from the browser to Pinata using a presigned upload URL
 * generated by our server. This bypasses CORS restrictions and Vercel's 4.5MB
 * serverless request body size limit, and avoids exposing any API credentials.
 */
async function uploadToPinataSigned(
  ...
  if (!signRes.ok) {
    let msg = `Failed to get upload signature (${signRes.status})`;
    try {
  ...
  const { url: signedUrl } = await signRes.json();

  // 2. Perform direct upload to Pinata using PUT
  ...
  if (!uploadRes.ok) {
    ...
    try {
      ...
      msg = e?.error?.details || e?.error || msg;
    } catch {}
    throw new Error(msg);
  }

  const resJson = await uploadRes.json();
  const cid = resJson?.data?.cid;
  if (!cid) {
