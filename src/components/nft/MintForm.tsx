...
const MintForm: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Format the gateway base URL from the public environment variables
      const gatewayEnv = process.env.NEXT_PUBLIC_PINATA_GATEWAY || 'gateway.pinata.cloud';
      let formattedGateway = gatewayEnv.startsWith('http') ? gatewayEnv : `https://${gatewayEnv}`;
      if (formattedGateway.endsWith('/')) formattedGateway = formattedGateway.slice(0, -1);
      if (!formattedGateway.includes('/ipfs')) formattedGateway = `${formattedGateway}/ipfs`;
      const pinataGateway = formattedGateway;

      if (coverFile) {
        const coverUrl2 = await uploadToPinataSigned(
          coverFile,
      ...
      const uploadResult = await uploadToPinataSigned(
        file,
        `${tokenId}_${file.name}`,
        pinataGateway
      );
